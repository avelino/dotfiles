...
in
{
  ...
  # Create Clojure configuration directory and files
  environment.etc = {
  ...
  # Create Clojure config directory and link configuration
  system.activationScripts.clojureSetup = {
    text = ''
      echo "Setting up Clojure configuration..."
      mkdir -p ${home}/.clojure
      rm -f ${home}/.clojure/deps.edn
      ln -sf /etc/clojure/deps.edn ${home}/.clojure/deps.edn
      chown -R avelino:staff ${home}/.clojure
    '';
    deps = [];
  };
} 